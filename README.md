...
- **v1.0.5**
  - 修复面板「设置点了没反应」：容器内 `ledctl` 路径写成了宿主机路径，导致 12 个写接口全部静默失败（还回成功提示）。
  - 时间表增加合法性校验（`25:00-30:00` 之类不再被静默吃掉）、支持显式清空。
  - 保存改为串行执行，消除「读-改-写」并发覆盖；配置读写加文件锁。
- **v1.0.4** — 容器化版本：面板与引擎全部在容器内运行，无需宿主机安装。
